...
module.exports.dbPostUser = user => {
  return new Promise((resolve, reject) => {
    ...
    db.run(
      `INSERT INTO users
      (first_name, last_name, account_created_date, last_login_date, street_address, city_address, state_code, zip_code, phone_number)
      VALUES('${user.first_name}', '${user.last_name}', '${current_date}', '${current_date}', '${user.street_address}',  '${user.city_address}', '${user.state_code}', '${user.zip_code}', '${user.phone_number}')`,
      function(err) {
        if (err) return reject(err);
